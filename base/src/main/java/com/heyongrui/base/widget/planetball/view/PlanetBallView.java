...
import com.heyongrui.base.widget.planetball.PlanetCalculator;
import com.heyongrui.base.widget.planetball.adapter.NullPlanetAdapter;
 ...
 * 3D行星球
 */
public class PlanetBallView extends ViewGroup implements Runnable, PlanetAdapter.OnDataSetChangeListener {
    ...
    public PlanetBallView(Context context) {
        super(context);
        init(context, null);
    }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.PlanetBallView);
            mode = typedArray.getInteger(R.styleable.PlanetBallView_autoScrollMode, MODE_DISABLE);
            setManualScroll(typedArray.getBoolean(R.styleable.PlanetBallView_manualScroll, true));
            mAngleX = typedArray.getFloat(R.styleable.PlanetBallView_startAngleX, 0.5f);
            mAngleY = typedArray.getFloat(R.styleable.PlanetBallView_startAngleY, 0.5f);
            setLightColor(typedArray.getColor(R.styleable.PlanetBallView_lightColor, Color.WHITE));
            setDarkColor(typedArray.getColor(R.styleable.PlanetBallView_darkColor, Color.BLACK));
            setRadiusPercent(typedArray.getFloat(R.styleable.PlanetBallView_radiusPercent, radiusPercent));
            setScrollSpeed(typedArray.getFloat(R.styleable.PlanetBallView_scrollSpeed, 2f));
            typedArray.recycle();
        }
        ...
        this.post(new Runnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < planetAdapter.getCount(); i++) {
                    ...
                    View view = planetAdapter.getView(getContext(), i, PlanetBallView.this);
        ...
        if (!view.hasOnClickListeners() && onTagClickListener != null) {
            view.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    onTagClickListener.onItemClick(PlanetBallView.this, v, position);
                }
            });
        }
    }
    ...
    public PlanetBallView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public PlanetBallView(Context context, AttributeSet attrs, int defStyleAttr) {
