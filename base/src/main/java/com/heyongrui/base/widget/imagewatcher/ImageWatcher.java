...
public class ImageWatcher<T> extends FrameLayout implements GestureDetector.OnGestureListener, ViewPager.OnPageChangeListener {
    ...
    protected int mErrorImageRes = R.drawable.placeholder_fail; // 图片加载失败站位图
    ...
    private OnPictureLongPressListener<T> pictureLongPressListener; // 图片长按回调
    ...
    public void setIndexProvider(IndexProvider<T> ip) {
        indexProvider = ip;
    ...
    public void setOnStateChangedListener(OnStateChangedListener<T> changedListener) {
        stateChangedListener = changedListener;
    }

    public void setOnPictureLongPressListener(OnPictureLongPressListener<T> listener) {
        pictureLongPressListener = listener;
    }

    public interface Loader<T> {
        void load(Context context, T t, LoadCallback lc);
    }
    ...
    public class DefaultIndexProvider implements IndexProvider {
        ...
        public void onPageChanged(ImageWatcher imageWatcher, int position, List dataList) {
            if (mDataList.size() > 1) {
                ...
                final String idxInfo = (position + 1) + " / " + mDataList.size();
    ...
    public interface OnStateChangedListener<T> {
        void onStateChangeUpdate(ImageWatcher imageWatcher, ImageView clicked, int position, T t, float animatedValue, int actionTag);

        void onStateChanged(ImageWatcher imageWatcher, int position, T t, int actionTag);
    }

    public interface OnPictureLongPressListener<T> {
        /**
         * @param v   当前被按的ImageView
         * @param t   当前ImageView加载展示的图片地址
         * @param pos 当前ImageView在展示组中的位置
         */
        void onPictureLongPress(ImageView v, T t, int pos); // 当前展示图片长按的回调
    }
     ...
     * @param dataList       被加载的图片url列表，数量必须大于等于 imageGroupList.size。 且顺序应当和imageGroupList保持一致
     */
    public void show(ImageView i, SparseArray<ImageView> imageGroupList, final List<T> dataList) {
        if (i == null || imageGroupList == null || dataList == null) {
            throw new NullPointerException("i[" + i + "]  imageGroupList[" + imageGroupList + "]  urlList[" + dataList + "]");
        }
        initPosition = -1;
        ...
        showInternal(i, imageGroupList, dataList);
    }

    private void showInternal(ImageView i, SparseArray<ImageView> imageGroupList, final List<T> dataList) {
        if (loader == null) {
        ...
        if (indexProvider != null) indexProvider.onPageChanged(this, initPosition, mDataList);
    }
    ...
    public T getDisplayingUri() {
        return mDataList != null ? mDataList.get(getCurrentPosition()) : null;
    }
    ...
    public void onLongPress(MotionEvent e) {
        if (pictureLongPressListener != null) {
            pictureLongPressListener.onPictureLongPress(iSource, mDataList.get(vPager.getCurrentItem()), vPager.getCurrentItem());
        }
    }
    ...
    public void onPageSelected(int position) {
        ...
        if (indexProvider != null) indexProvider.onPageChanged(this, position, mDataList);
    ...
    class ImagePagerAdapter extends PagerAdapter {
        ...
        private boolean setDefaultDisplayConfigs(final ImageView imageView, final int pos, boolean hasPlayBeginAnimation) {
            ...
            loader.load(imageView.getContext(), mDataList.get(pos), new LoadCallback() {
                @Override
